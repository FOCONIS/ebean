...
public class TestSoftDeletePagingList extends BaseTestCase {
  ...
  public void test() {
    ...
    assertThat(sql.get(0)).contains("and coalesce(t0.deleted,"+quoteValue("false")+")="+quoteValue("false")+"; --bind(SoftDelPaged-%)");

    assertThat(sql.get(1)).contains("where t0.s3url like ");
    assertThat(sql.get(1)).contains("and coalesce(t0.deleted,"+quoteValue("false")+")="+quoteValue("false")+" order by t0.id");
  }
}
