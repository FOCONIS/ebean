...
public class TestOneToOneOptionalRelationship extends BaseTestCase {
  ...
  public void test() {
    ...
    Assert.assertTrue(sql.contains(" from " + SCHEMA_PREFIX + "oto_account t0 left join " + SCHEMA_PREFIX + "oto_user t1 on t1.account_id = t0.id  where t0.id = ?"));
  }
  ...
  public void testWithUser() {
    ...
    Assert.assertTrue(lazyLoadSql.contains("select t0.id, t0.name, t0.version, t0.when_created, t0.when_modified, t0.account_id from " + SCHEMA_PREFIX + "oto_user t0 where t0.id = ?"));
  }
