...
public class TestElementCollectionEmbeddedMap extends BaseTestCase {
  ...
  private void updateBoth(EcbmPerson bean) {
    ...
    assertThat(sql.get(2)).contains("insert into ecbm_person_phone_numbers (person_id,_key,country_code,area,number)");
  ...
  private void updateOnlyCollection(EcbmPerson bean) {
    ...
    assertThat(sql.get(1)).contains("insert into ecbm_person_phone_numbers (person_id,_key,country_code,area,number) values (?,?,?,?,?)");
