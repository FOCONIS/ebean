...
public class TestHstore extends BaseTestCase {
  ...
  void update_when_notDirty() {
    ...
    assertThat(sql.get(0)).contains("update " + SCHEMA_PREFIX + "ebasic_hstore set name=?, version=? where");
  }
  ...
  void update_when_dirty() {
    ...
    assertThat(sql.get(0)).contains("update " + SCHEMA_PREFIX + "ebasic_hstore set name=?, map=?, version=? where id=? and version=?");
  }
