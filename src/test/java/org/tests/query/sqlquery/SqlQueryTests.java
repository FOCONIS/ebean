...
public class SqlQueryTests extends BaseTestCase {
  ...
  public void maxRows() {
    ...
    if (isSqlServer()) {
      assertThat(sql.get(0)).contains(" Select * from " + SCHEMA_PREFIX + "o_order order by id offset 0 rows fetch next 10 rows only;");
    } else {
  ...
  public void maxRows_withParam() {
    ...
    if (isSqlServer()) {
      assertThat(sql.get(0)).contains("select * from " + SCHEMA_PREFIX + "o_order where o_order.id > ? order by id  offset 0 rows fetch next 10 rows only;");
    } else {
