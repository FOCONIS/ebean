...
public class TestQueryIsNull extends BaseTestCase {
  ...
  public void queryShouldContainIsNullOnColumn() {
    ...
    assertTrue(sqlOf(query).contains("name is null"));
  }
  ...
  public void isNotNull_when_OneToMany_expect_existsSubquery() {
    ...
    assertTrue(sqlOf(query).contains(" where exists (select 1 from o_order_detail where order_id = t0.id)"));
  }
  ...
  public void isNull_when_OneToMany_expect_notExistsSubquery() {
    ...
    assertTrue(sqlOf(query).contains(" where not exists (select 1 from o_order_detail where order_id = t0.id)"));
  }
  ...
  public void isEmpty_when_ManyToMany_expect_notExistsSubqueryAndNoJoin() {
    ...
    assertTrue(sqlOf(query).contains("from mt_role t0 where not exists (select 1 from mt_role_permission where mt_role_id = t0.id)"));
  }
  ...
  public void isNotEmpty_when_ManyToMany_expect_existsSubqueryAndNoJoin() {
    ...
    assertTrue(sqlOf(query).contains("from mt_role t0 where exists (select 1 from mt_role_permission where mt_role_id = t0.id)"));
  }
