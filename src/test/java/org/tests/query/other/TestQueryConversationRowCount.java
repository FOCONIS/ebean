...
public class TestQueryConversationRowCount extends BaseTestCase {
  ...
  public void test() {
    ...
    assertThat(countSql).contains("select count(*) from ( select distinct t0.id from " + SCHEMA_PREFIX + "c_conversation t0 left join " + SCHEMA_PREFIX + "c_participation u1 on u1.conversation_id = t0.id  where t0.group_id = ?  and ((t0.isopen = ?  and u1.user_id = ? )  or t0.isopen = ? )");
  }
