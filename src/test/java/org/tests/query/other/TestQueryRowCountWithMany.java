...
public class TestQueryRowCountWithMany extends BaseTestCase {
  ...
  public void test() {
    ...
    assertThat(trimSql(sqlLogged.get(1), 1)).contains(
      "select count(*) from ( select distinct t0.id from " + SCHEMA_PREFIX + "o_order t0 join " + SCHEMA_PREFIX + "o_order_detail u1 on u1.order_id = t0.id  where u1.product_id = ? )");
  ...
  public void testWithFirstRowsMaxRows() {
    ...
    assertThat(trimSql(sqlLogged.get(0), 1)).contains("select count(*) from ( select distinct t0.id from " + SCHEMA_PREFIX + "o_order t0 join " + SCHEMA_PREFIX + "o_order_detail u1 on u1.order_id = t0.id  where u1.product_id = ? )");
