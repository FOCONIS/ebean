...
public class TestQueryFilterMany extends BaseTestCase {
  ...
  public void testNestedFilterMany() {
    ...
    assertThat(sql.get(0)).contains(" from " + SCHEMA_PREFIX + "o_customer t0; --bind()");
    assertThat(sql.get(1)).contains(" from " + SCHEMA_PREFIX + "contact t0 where (t0.customer_id) in");
    assertThat(sql.get(1)).contains(" and t0.first_name is not null");
    assertThat(sql.get(2)).contains(" from " + SCHEMA_PREFIX + "contact_note t0 where (t0.contact_id) in");
