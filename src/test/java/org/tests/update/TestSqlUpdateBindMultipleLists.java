...
public class TestSqlUpdateBindMultipleLists extends BaseTestCase {
  ...
  public void test() {
    ...
    SqlUpdate sqlUpdate = Ebean.createSqlUpdate("delete from ${tenant_schema}.o_customer where id in (:ids)");

    sqlUpdate.setParameter("ids", asList(9991, 9992, 9993));
    sqlUpdate.execute();
    assertEquals("delete from ${tenant_schema}.o_customer where id in (?,?,?)", sqlUpdate.getGeneratedSql());
  ...
  public void test_multipleLists() {
    ...
    SqlUpdate sqlUpdate = Ebean.createSqlUpdate("delete from ${tenant_schema}.o_customer where id in (:ids) and name in (:names)");

    sqlUpdate.setParameter("ids", asList(9991, 9992, 9993));
    sqlUpdate.setParameter("names", asList("rob", "jim"));
    sqlUpdate.execute();
    assertEquals("delete from ${tenant_schema}.o_customer where id in (?,?,?) and name in (?,?)", sqlUpdate.getGeneratedSql());
