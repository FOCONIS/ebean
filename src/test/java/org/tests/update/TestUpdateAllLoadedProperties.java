...
public class TestUpdateAllLoadedProperties extends BaseTestCase {
  ...
  public void test() {
    ...
    try {
      ...
      assertTrue(loggedSql.get(0), loggedSql.get(0).contains("update " + SCHEMA_PREFIX + "e_basicver set name=?, description=?, other=?, last_update=? where id=? and last_update=?; --bind("));
      assertTrue(loggedSql.get(1), loggedSql.get(1).contains("update " + SCHEMA_PREFIX + "e_basicver set name=?, description=?, other=?, last_update=? where id=? and last_update=?; --bind("));
  ...
  private void testPartiallyLoaded(Integer id1, Integer id2) {
    ...
    assertThat(loggedSql.get(0)).contains("update " + SCHEMA_PREFIX + "e_basicver set name=?, other=?, last_update=? where id=?; --bind(");
    assertThat(loggedSql.get(1)).contains("update " + SCHEMA_PREFIX + "e_basicver set name=?, other=?, last_update=? where id=?; --bind(");
