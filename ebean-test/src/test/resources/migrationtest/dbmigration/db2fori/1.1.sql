...
create unique index uq_migtest_e_basic_description on migtest_e_basic(description) exclude null keysalter table migtest_e_basic add constraint fk_migtest_e_basic_user_id foreign key (user_id) references migtest_e_user (id) on delete restrict;
alter table migtest_e_basic add constraint ck_migtest_e_basic_progress check ( progress in (0,1,2));
create unique index uq_migtest_e_basic_status_indextest1 on migtest_e_basic(status,indextest1) exclude null keyscreate unique index uq_migtest_e_basic_name on migtest_e_basic(name) exclude null keyscreate unique index uq_migtest_e_basic_indextest4 on migtest_e_basic(indextest4) exclude null keyscreate unique index uq_migtest_e_basic_indextest5 on migtest_e_basic(indextest5) exclude null keysalter table migtest_oto_child add constraint fk_migtest_oto_child_master_id foreign key (master_id) references migtest_oto_master (id) on delete restrict;
