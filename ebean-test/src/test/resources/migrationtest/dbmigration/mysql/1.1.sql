...
alter table foo.migtest_e_history add column sys_period_start datetime(6) default now(6);
alter table foo.migtest_e_history add column sys_period_end datetime(6);
...
lock tables foo.migtest_e_history write;
delimiter $$
create trigger foo.migtest_e_history_history_upd before update on foo.migtest_e_history for each row begin
    insert into foo.migtest_e_history_history (sys_period_start,sys_period_end,id, test_string) values (OLD.sys_period_start, now(6),OLD.id, OLD.test_string);
    set NEW.sys_period_start = now(6);
end$$
delimiter $$
create trigger foo.migtest_e_history_history_del before delete on foo.migtest_e_history for each row begin
    insert into foo.migtest_e_history_history (sys_period_start,sys_period_end,id, test_string) values (OLD.sys_period_start, now(6),OLD.id, OLD.test_string);
end$$
unlock tables;
