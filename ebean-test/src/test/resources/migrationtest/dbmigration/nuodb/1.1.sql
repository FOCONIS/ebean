...
alter table foo.migtest_e_history add column sys_period_start datetime(6) default now();
...
delimiter $$
create or replace trigger foo.migtest_e_history_history_upd for foo.migtest_e_history before update for each row as 
    NEW.sys_period_start = greatest(current_timestamp, date_add(OLD.sys_period_start, interval 1 microsecond));
    insert into foo.migtest_e_history_history (sys_period_start,sys_period_end,id, test_string) values (OLD.sys_period_start, NEW.sys_period_start,OLD.id, OLD.test_string);
end_trigger;
$$

delimiter $$
create or replace trigger foo.migtest_e_history_history_del for foo.migtest_e_history before delete for each row as
    insert into foo.migtest_e_history_history (sys_period_start,sys_period_end,id, test_string) values (OLD.sys_period_start, NEW.sys_period_start,OLD.id, OLD.test_string);
end_trigger;
$$
