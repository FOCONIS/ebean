...
-- drop dependencies
alter table migtest_fk_cascade drop constraint if existsfk_migtest_fk_cascade_one_idalter table migtest_fk_none drop constraint if existsfk_migtest_fk_none_one_idalter table migtest_fk_none_via_join drop constraint if existsfk_migtest_fk_none_via_join_one_idalter table migtest_fk_set_null drop constraint if existsfk_migtest_fk_set_null_one_idalter table migtest_e_basic drop constraint if exists ck_migtest_e_basic_status;
...
alter table migtest_e_basic drop constraint if existsfk_migtest_e_basic_user_idalter table migtest_e_basic drop constraint uq_migtest_e_basic_status_indextest1;
...
-- indices/constraints
alter table migtest_ckey_detail drop constraint if existsfk_migtest_ckey_detail_parentalter table migtest_fk_cascade add constraint fk_migtest_fk_cascade_one_id foreign key (one_id) references migtest_fk_cascade_one (id) on delete cascade on update restrict;
